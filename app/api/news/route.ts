...
function deriveCategory(text: string): string {
  const lower = text.toLowerCase();
  if (/bitcoin|ethereum|crypto|blockchain|defi|nft|btc|eth|solana/.test(lower))
    return "Crypto";
  if (
    /oil|gold|commodity|commodities|wheat|copper|silver|crude|metal|energy/.test(
      lower,
    )
  )
    return "Commodities";
  if (
    /fed|federal reserve|inflation|gdp|macro|interest rate|economy|treasury|cpi|fomc|recession/.test(
      lower,
    )
  )
    return "Macro";
  if (
    /apple|amazon|google|alphabet|microsoft|meta|nvidia|tesla|openai|ai |artificial intelligence|software|semiconductor|chip/.test(
      lower,
    )
  )
    return "Tech";
...
function deriveImpact(text: string, sentiment: Sentiment): number {
  ...
  return Math.min(
    10,
    parseFloat((base + hits * 0.45 + Math.random() * 1.2).toFixed(1)),
  );
}
...
export async function GET(req: NextRequest) {
  ...
  if (!apiKey || apiKey === "your_finnhub_api_key") {
    return errorResponse(
      "CONFIG_ERROR",
      "FINNHUB_API_KEY not configured or invalid",
      { status: 500 },
    );
  }
  ...
  try {
    ...
    const items = raw
      .filter(
        (
          ...
          Pick<(typeof raw)[number], "headline" | "summary" | "datetime">
        > &
          (typeof raw)[number] =>
    ...
    const message =
      error instanceof Error ? error.message : "Failed to fetch news feed";
