...
import {
    Users, MessageSquare, TrendingUp, ThumbsUp, ThumbsDown, Flame, Star,
    Trophy, ChevronUp, ChevronDown, Pin, Share2, BookOpen, BarChart2,
    ArrowRight, Tag, Bell, Search, Filter, Clock, CheckCircle,
    Zap, Shield, Globe, Hash, Send, Award, Target, PieChart, Activity
} from 'lucide-react';
...
/* ── Types ─────────────────────────────────────────────────────────── */
type Post = {
    id: number; author: string; avatar: string; role: string; roleColor: string;
    time: string; title: string; body: string; tags: string[];
    upvotes: number; downvotes: number; replies: number; views: number;
    pinned?: boolean; flair?: string; flairColor?: string;
    userVote: 'up' | 'down' | null;
};
type Reply = { author: string; avatar: string; role: string; roleColor: string; time: string; body: string; upvotes: number; };
...
const POSTS: Post[] = [
    {
        id: 1, pinned: true,
        author: 'AlphaTrader99', avatar: '🦅', role: 'Elite Trader', roleColor: 'text-amber-400',
        time: '2h ago',
        flair: 'Analysis', flairColor: 'bg-blue-500/20 text-blue-400 border-blue-500/30',
        ...
        upvotes: 312, downvotes: 18, replies: 47, views: 2840, userVote: null,
    },
    {
        id: 2,
        author: 'QuantWhiz', avatar: '🤖', role: 'Quant Analyst', roleColor: 'text-blue-400',
        time: '5h ago',
        flair: 'Strategy', flairColor: 'bg-violet-500/20 text-violet-400 border-violet-500/30',
        title: 'My momentum screener flagged 4 stocks this week — sharing the setup',
        ...
        upvotes: 218, downvotes: 9, replies: 32, views: 1620, userVote: null,
    },
    {
        id: 3,
        author: 'NiftyNinja', avatar: '⚡', role: 'Options Expert', roleColor: 'text-emerald-400',
        time: '8h ago',
        flair: 'Education', flairColor: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30',
        ...
        upvotes: 445, downvotes: 4, replies: 68, views: 5210, userVote: null,
    },
    {
        id: 4,
        author: 'BullSignal_K', avatar: '📊', role: 'Senior Member', roleColor: 'text-slate-400',
        time: '12h ago',
        flair: 'Analysis', flairColor: 'bg-blue-500/20 text-blue-400 border-blue-500/30',
        ...
        upvotes: 174, downvotes: 31, replies: 29, views: 1890, userVote: null,
    },
    {
        id: 5,
        author: 'DeepikaTrades', avatar: '🌿', role: 'Rising Star', roleColor: 'text-emerald-300',
        time: '1d ago',
        flair: 'Learn Together', flairColor: 'bg-amber-500/20 text-amber-400 border-amber-500/30',
        title: 'Beginner question: why does a stock gap up and then immediately fall?',
        ...
        upvotes: 198, downvotes: 2, replies: 41, views: 2140, userVote: null,
    },
    {
        id: 6,
        author: 'SectorRotator', avatar: '🔄', role: 'Senior Member', roleColor: 'text-slate-400',
        time: '1d ago',
        flair: 'Strategy', flairColor: 'bg-violet-500/20 text-violet-400 border-violet-500/30',
        ...
        upvotes: 145, downvotes: 22, replies: 18, views: 1340, userVote: null,
    },
];

const MOCK_REPLIES: Reply[] = [
    { author: 'TrendFollower', avatar: '📈', role: 'Member', roleColor: 'text-slate-400', time: '1h ago', body: 'Agree on the 200 EMA watch. I\'d also keep an eye on the India VIX — if it drops below 14, that\'s a strong signal the fear is dissipating and the bounce has legs.', upvotes: 42 },
    { author: 'ShortTermSam', avatar: '⚡', role: 'Active Trader', roleColor: 'text-blue-400', time: '45m ago', body: 'I\'m less bullish. PCR jumped to 1.4 — option sellers are positioned for further downside. The FII selling trend has been consistent for 6 weeks. I think 22,400 hits before 23,200.', upvotes: 28 },
    { author: 'LongTermLisa', avatar: '🌱', role: 'Investor', roleColor: 'text-emerald-400', time: '30m ago', body: 'SIP investors shouldn\'t be watching charts daily. This volatility is a gift — stick to your allocation, don\'t switch to cash waiting for the "bottom."', upvotes: 67 },
];

const TOP_MEMBERS = [
    { name: 'AlphaTrader99', avatar: '🦅', role: 'Elite Trader', posts: 1240, likes: 8920, badge: '🥇' },
    { name: 'NiftyNinja',    avatar: '⚡', role: 'Options Expert', posts: 890, likes: 6740, badge: '🥈' },
    { name: 'QuantWhiz',     avatar: '🤖', role: 'Quant Analyst', posts: 640, likes: 4810, badge: '🥉' },
    { name: 'BullSignal_K',  avatar: '📊', role: 'Senior Member',  posts: 412, likes: 3120, badge: '4' },
    { name: 'DeepikaTrades', avatar: '🌿', role: 'Rising Star',    posts: 98,  likes: 1890, badge: '5' },
];

const TRENDING_TAGS = ['Nifty50', 'Options', 'Banking', 'IT Sector', 'Budget2026', 'RSI', 'HDFC Bank', 'Momentum', 'SIP', 'FII Data'];
...
const PostDetail = ({ post, onClose }: { post: Post; onClose: () => void }) => {
    ...
    const [votes, setVotes] = useState({ up: post.upvotes, down: post.downvotes, vote: post.userVote });
    const [replyText, setReplyText] = useState('');
    const [replies, setReplies] = useState<Reply[]>(MOCK_REPLIES);

    const handleVote = (dir: 'up' | 'down') => {
        setVotes(v => {
            if (v.vote === dir) return { up: post.upvotes, down: post.downvotes, vote: null };
            return {
        ...
        void trackActivity('community_post_vote', { postId: post.id, direction: dir });
    };
    ...
    const submitReply = () => {
        ...
        setReplies(r => [{
            author: 'You', avatar: '👤', role: 'Member', roleColor: 'text-amber-400',
            time: 'just now', body: nextReply, upvotes: 0,
        }, ...r]);
        setReplyText('');
        void trackActivity('community_reply_created', { postId: post.id, replyLength: nextReply.length });
    };
            ...
            <motion.div initial={{ opacity: 0, y: 30, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.97 }}
                ...
                <div className="flex items-center justify-between p-3 sm:p-4 border-b border-slate-800/60">
                    <div className="flex items-center gap-2 min-w-0">
                        {post.flair && (
                            <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded border shrink-0 ${post.flairColor}`}>{post.flair}</span>
                        )}
                        <span className="text-[10px] sm:text-[11px] text-slate-500 truncate">{post.replies} replies · {post.views.toLocaleString()} views</span>
                    </div>
                    <button onClick={onClose} className="text-slate-500 hover:text-white text-xs sm:text-sm transition-colors px-2 py-1 rounded hover:bg-slate-800 shrink-0 ml-2">✕ Close</button>
                </div>

                <div className="p-3 sm:p-5">
                    {/* Author */}
                    <div className="flex items-center gap-2.5 mb-3 sm:mb-4">
                        <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-lg sm:text-xl shrink-0">{post.avatar}</div>
                        <div className="min-w-0">
                            <div className={`text-sm font-black truncate ${post.roleColor}`}>{post.author}</div>
                            <div className="text-[10px] sm:text-xs text-slate-600 whitespace-nowrap">{post.role} · {post.time}</div>
                        </div>
                        {post.pinned && <div className="ml-auto flex items-center gap-1 text-[10px] sm:text-xs text-amber-500 shrink-0"><Pin size={11} /> Pinned</div>}
                    </div>

                    <h2 className="text-base sm:text-xl font-black text-white mb-2 sm:mb-3 leading-snug">{post.title}</h2>
                    <p className="text-xs sm:text-sm text-slate-400 leading-relaxed mb-3 sm:mb-4">{post.body}</p>

                    <div className="flex flex-wrap gap-1.5 mb-5">
                        {post.tags.map(t => (
                            <span key={t} className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-slate-800 text-slate-500 border border-slate-700/50 flex items-center gap-1">
                                <Hash size={9} />{t}
                            </span>
                        ))}
                    </div>
                    ...
                    <div className="mt-3 sm:mt-4 mb-4 sm:mb-5">
                        <div className="flex gap-2">
                            <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-amber-500/15 border border-amber-500/30 flex items-center justify-center text-xs sm:text-sm shrink-0">👤</div>
                    ...
                    <div className="space-y-2.5 sm:space-y-3">
                        {replies.map((r, i) => (
                            <div key={i} className="flex gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-xl bg-slate-900/40 border border-slate-800/40">
                                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-xs sm:text-sm shrink-0">{r.avatar}</div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5 mb-1.5">
                                        <span className={`text-xs font-black whitespace-nowrap ${r.roleColor}`}>{r.author}</span>
                                        <span className="text-[10px] text-slate-600 whitespace-nowrap">{r.role}</span>
                                        <span className="text-[10px] text-slate-700 whitespace-nowrap ml-auto">{r.time}</span>
                                    </div>
                                    <p className="text-[11px] sm:text-xs text-slate-400 leading-relaxed">{r.body}</p>
...
const PostCard = ({ post, onClick }: { post: Post; onClick: () => void }) => {
    ...
    const handleVote = (e: React.MouseEvent, dir: 'up' | 'down') => {
        e.stopPropagation();
        if (dir === 'up') setVotes(v => v.vote === 'up' ? { up: post.upvotes, vote: null } : { up: post.upvotes + 1, vote: 'up' });
    };
    ...
    return (
        <motion.div variants={fadeUp} onClick={onClick}
            ...
            <div className="flex items-start min-[420px]:items-center gap-x-2 gap-y-1.5 mb-3 flex-wrap">
                {post.pinned && (
                    <span className="flex items-center gap-1 text-[10px] font-bold text-amber-500 uppercase tracking-wider"><Pin size={11} /> Pinned</span>
                )}
                {post.flair && (
                    <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded border ${post.flairColor}`}>{post.flair}</span>
                )}
                <div className="flex items-center gap-1.5 min-w-0 min-[420px]:ml-auto">
                    <div className="w-6 h-6 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-xs">{post.avatar}</div>
                    <span className={`text-xs font-bold truncate ${post.roleColor}`}>{post.author}</span>
                    <span className="text-[10px] text-slate-600 whitespace-nowrap">· {post.time}</span>
                </div>
            </div>

            {/* Title & body */}
            <h3 className="text-sm font-black text-white mb-1.5 leading-snug group-hover:text-amber-300 transition-colors">{post.title}</h3>
            <p className="text-xs text-slate-500 leading-relaxed mb-3 line-clamp-2">{post.body}</p>

            {/* Tags */}
            <div className="flex flex-wrap gap-1.5 mb-4">
                {post.tags.map(t => (
                    <span key={t} className="text-[10px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-slate-800 text-slate-600 border border-slate-700/50 flex items-center gap-1">
                        <Hash size={9} />{t}
                    </span>
                ))}
            </div>
            ...
            <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
                ...
                <span className="text-xs text-slate-600 flex items-center gap-1"><ThumbsDown size={11} /> {post.downvotes}</span>
                <span className="text-xs text-slate-600 flex items-center gap-1"><MessageSquare size={11} /> {post.replies} replies</span>
                <span className="text-xs text-slate-600 flex items-center gap-1 min-[420px]:ml-auto"><Activity size={11} /> {post.views.toLocaleString()} views</span>
            </div>
        </motion.div>
    );
};
...
/* ── New Post Modal ─────────────────────────────────────────────────── */
const NewPostModal = ({ onClose, onSubmit }: { onClose: () => void; onSubmit: (post: Post) => void }) => {
    const [title, setTitle] = useState('');
    const [body,  setBody]  = useState('');
    const [flair, setFlair] = useState('Analysis');

    const flairs = [
        { label: 'Analysis',    color: 'bg-blue-500/20 text-blue-400 border-blue-500/30'   },
        { label: 'Strategy',    color: 'bg-violet-500/20 text-violet-400 border-violet-500/30' },
        { label: 'Education',   color: 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' },
        { label: 'Learn Together', color: 'bg-amber-500/20 text-amber-400 border-amber-500/30' },
        { label: 'Discussion',  color: 'bg-slate-700/60 text-slate-300 border-slate-700'    },
    ];
    ...
    const handleSubmit = () => {
        ...
        onSubmit({
            id: Date.now(), author: 'You', avatar: '👤', role: 'Member', roleColor: 'text-amber-400',
            time: 'just now', title: title.trim(), body: body.trim(),
            tags: [], upvotes: 0, downvotes: 0, replies: 0, views: 0,
            flair, flairColor: flairEntry.color, userVote: null,
        });
        onClose();
    };
                ...
                <div className="p-5 space-y-4">
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {flairs.map(f => (
                                <button key={f.label} onClick={() => setFlair(f.label)}
                    ...
                    <div>
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2 block">Title</label>
                        <input value={title} onChange={e => setTitle(e.target.value)} placeholder="What's your take or question?"
                    ...
                    <div>
                        <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2 block">Body</label>
                        <textarea value={body} onChange={e => setBody(e.target.value)} rows={5} placeholder="Share your analysis, idea, or question in detail…"
...
const Community = () => {
    ...
    const filtered = posts.filter(p => {
        const matchSearch = !searchQuery || p.title.toLowerCase().includes(searchQuery.toLowerCase()) || p.body.toLowerCase().includes(searchQuery.toLowerCase());
        const matchCategory = activeCategory === 'all' || (
            activeCategory === 'analysis' && p.flair === 'Analysis' ||
            activeCategory === 'learning' && (p.flair === 'Education' || p.flair === 'Learn Together') ||
            activeCategory === 'picks'    && p.tags.some(t => ['Banking', 'IT Sector', 'Nifty50'].includes(t)) ||
    ...
    const sorted = [...filtered].sort((a, b) => {
        if (sortBy === 'hot') return (b.upvotes - b.downvotes + b.replies * 5) - (a.upvotes - a.downvotes + a.replies * 5);
            ...
            <div className="max-w-7xl mx-auto relative">
                ...
                <motion.div variants={stagger} initial="hidden" animate="show" className="text-center mb-8 sm:mb-12">
                    ...
                    <motion.h1 variants={fadeUp} className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-white mb-3 sm:mb-4 md:mb-5 leading-tight">
                        Trade Smarter,{' '}
                        <span className="bg-linear-to-r from-amber-400 to-amber-300 bg-clip-text text-transparent">Together</span>
                    </motion.h1>
                    <motion.p variants={fadeUp} className="text-xs sm:text-base md:text-lg lg:text-xl text-slate-400 max-w-2xl mx-auto mb-5 sm:mb-8 md:mb-10">
                        Discuss market analysis, share trade ideas, and learn from India&apos;s most active community of traders and investors.
                    </motion.p>
                    {/* Stats strip */}
                    <motion.div variants={fadeUp} className="grid grid-cols-2 min-[420px]:grid-cols-4 justify-items-center gap-3 sm:gap-6 mb-6 sm:mb-8">
                        {STATS.map(s => (
                            <div key={s.label} className="flex items-center gap-1.5">
                                <s.Icon size={13} className="text-amber-400 shrink-0" />
                                <span className="text-white font-extrabold text-sm sm:text-lg">{s.val}</span>
                                <span className="text-slate-500 text-xs sm:text-base">{s.label}</span>
                            </div>
                        ))}
                    </motion.div>
                    <motion.button variants={fadeUp} onClick={() => openComposer('hero')}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 items-start">
                    ...
                    <div className="xl:col-span-1 space-y-4 xl:sticky xl:top-24">
                        ...
                        <div className="bg-[#0b1120] border border-slate-800/60 rounded-2xl p-4">
                            <p className="text-xs font-black text-slate-500 uppercase tracking-wider mb-3">Browse</p>
                                ...
                                {CATEGORIES.map(cat => {
                                    ...
                                    return (
                                        <button key={cat.id} onClick={() => setActiveCategory(cat.id)}
                                            ...
                                            <CIcon size={13} className={active ? 'text-amber-400' : ''} />
                                            <span className="flex-1 text-sm font-semibold">{cat.label}</span>
                                            <span className={`text-xs ${active ? 'text-amber-500' : 'text-slate-700'}`}>{cat.count}</span>
                                        </button>
                                    );
                                })}
                            </div>
                            ...
                            <ol className="space-y-2">
                                {[
                                    ...
                                    <li key={i} className="flex items-start gap-2">
                                        ...
                                        <span className="text-xs text-slate-500 leading-snug">{rule}</span>
                                    </li>
                                ))}
                            </ol>
                        </div>
                    ...
                    <div className="lg:col-span-2 xl:col-span-2 flex flex-col gap-4">
                        {/* Scrollable Posts */}
                        <div className="flex flex-col gap-4 max-h-none xl:max-h-[calc(100vh-5rem)] overflow-y-auto pr-1 [scrollbar-width:thin] [scrollbar-color:#334155_transparent]">
                        {/* Toolbar */}
                        ...
                        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-3">
                            {sorted.length > 0 ? sorted.map(post => (
                                ...
                                <div className="text-center py-16 text-slate-600">
                                    ...
                                    <p className="text-sm">No posts found. Be the first to start a discussion!</p>
                                </div>
                            )}
                        </motion.div>
                        </div>

                        <button onClick={() => openComposer('feed-footer')}
                    ...
                    <div className="xl:col-span-1 space-y-4 xl:sticky xl:top-24">
                        ...
                        <div className="bg-[#0b1120] border border-slate-800/60 rounded-2xl p-4">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <p className="text-xs font-black text-white uppercase tracking-wider">Top Contributors</p>
                            </div>
                            <div className="space-y-3">
                                {TOP_MEMBERS.map(m => (
                                    <div key={m.name} className="flex items-center gap-2.5">
                                        <div className="w-7 h-7 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-sm">{m.avatar}</div>
                                        <div className="flex-1 min-w-0">
                                            <div className="text-xs font-bold text-slate-300 truncate">{m.name}</div>
                                            <div className="text-[10px] text-slate-600">{m.posts} posts · {m.likes.toLocaleString()} likes</div>
                                        </div>
                                        <span className="text-sm">{isNaN(Number(m.badge)) ? m.badge : `#${m.badge}`}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        {/* Weekly Insight */}
                        <div className="bg-[#0b1120] border border-amber-500/20 rounded-2xl p-4" style={{ background: 'linear-gradient(135deg, #0d1a2e 0%, #0f1e35 100%)' }}>
                            <div className="flex items-center gap-2 mb-2">
                                <Star size={15} className="text-amber-400" />
                                <p className="text-xs font-black text-amber-400 uppercase tracking-wider">This Week&apos;s Insight</p>
                            </div>
                            <p className="text-sm text-slate-300 leading-relaxed mb-2">
                                &ldquo;The market punishes impatience severely and rewards discipline generously. The best trade is often no trade.&rdquo;
                            </p>
                            <p className="text-xs text-slate-600">— Shared by AlphaTrader99</p>
                        </div>
                        ...
                        <div className="bg-[#0b1120] border border-slate-800/60 rounded-2xl p-5 mt-auto">
                            <div className="flex items-center gap-2 mb-3">
                                ...
                                <p className="text-xs font-black text-white uppercase tracking-wider">Daily Challenge</p>
                            </div>
                            <p className="text-base text-slate-300 mb-3 leading-snug font-semibold">
                                Name one sector outperforming Nifty 50 today and explain why in 3 sentences or less.
                            </p>
                            <div className="flex items-center gap-2 mb-3">
                                <Award size={14} className="text-amber-400" />
                                <span className="text-xs text-slate-500">84 answers · Ends in 6h</span>
                            </div>
                            <button onClick={() => openComposer('daily-challenge')}
