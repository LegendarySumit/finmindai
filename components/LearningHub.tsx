...
const GLOSSARY = [
  {
    term: "Bull Market",
    def: "A market condition where prices are rising or expected to rise, typically by 20%+ from recent lows.",
  },
  {
    term: "Bear Market",
    def: "A market condition where prices fall 20%+ from recent highs, often accompanied by widespread pessimism.",
  },
  {
    term: "P/E Ratio",
    def: "Price-to-Earnings ratio. Measures how much investors pay per ₹1 of earnings. Lower can mean undervalued.",
  },
  {
    term: "Market Cap",
    def: "Total market value of a company = share price × total shares outstanding.",
  },
  {
    term: "Liquidity",
  ...
  {
    term: "Volatility",
    def: "The degree of price variation over time. High volatility = higher risk and potential reward.",
  },
  {
    term: "Dividend Yield",
    def: "Annual dividend per share divided by the share price, expressed as a percentage.",
  },
  {
    term: "Short Selling",
    def: "Borrowing and selling a stock you don't own, hoping to buy it back cheaper later.",
  },
  {
    term: "Blue Chip",
    def: "Shares of large, well-established, financially stable companies with a long track record.",
  },
  {
    term: "IPO",
    def: "Initial Public Offering — when a private company offers its shares to the public for the first time.",
  },
  {
    term: "Circuit Breaker",
    def: "A market mechanism that temporarily halts trading when prices fall too sharply (e.g., 5%, 10%).",
  },
  {
    term: "CAGR",
    def: "Compound Annual Growth Rate — the mean annual growth rate of an investment over a specified time.",
  },
];
...
const FAQS = [
  {
    ...
    a: "You can start with as little as ₹500–₹1,000 using mutual funds or fractional shares. The key is to start early and invest consistently.",
  },
  {
    q: "What is the safest investment for a beginner?",
    a: "Index funds and large-cap ETFs are often recommended for beginners as they offer instant diversification with lower risk than individual stocks.",
  },
  {
    q: "How long does it take to learn trading?",
    a: "Grasping the basics takes a few weeks. Becoming consistently profitable typically requires 1–2 years of learning, paper trading, and real experience.",
  },
  {
    q: "What is the difference between NSE and BSE?",
    a: "NSE (National Stock Exchange) and BSE (Bombay Stock Exchange) are India's two main stock exchanges. NSE is higher in volume; both list the same major companies.",
  },
  {
    q: "Is trading better than long-term investing?",
    a: "Not necessarily — 90%+ of beginner traders lose money. Long-term investing in diversified portfolios historically outperforms most active trading strategies.",
  },
];
...
const CourseCard = ({
  ...
  course: (typeof COURSES)["beginner"][0];
  trackColor: string;
  onClick: () => void;
}) => {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2 mb-1">
          <h4 className="text-[13px] font-bold text-white leading-snug group-hover:text-amber-300 transition-colors">
            {course.title}
          </h4>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-1.5">
          {course.tags.map((t) => (
            <span
              key={t}
              className="text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded bg-slate-800 text-slate-500 border border-slate-700/50"
            >
              {t}
            </span>
          ))}
...
const LearningHub = () => {
  ...
  const handleCourseOpen = (courseId: string) => {
    ...
    void trackActivity("learning_course_opened", {
      courseId,
      track: activeTrack,
    });
  };
  ...
  if (activeCourseId) {
    ...
    return (
      <CoursePlayer
        ...
        onBack={() => {
          void trackActivity("learning_course_closed", {
            courseId: activeCourseId,
            track: activeTrack,
          });
  ...
  return (
    <section className="min-h-screen bg-finance-darker relative overflow-hidden pt-12 sm:pt-14 md:pt-16 pb-10 sm:pb-14 md:pb-20 lg:pb-24 px-2 sm:px-3 md:px-4 lg:px-6 xl:px-8">
      {/* bg glows */}
      <div
        className="pointer-events-none absolute top-0 left-1/4 w-150 h-100 rounded-full blur-3xl opacity-20"
        style={{
          background: "radial-gradient(circle, #3b82f6 0%, transparent 70%)",
        }}
      />
      <div
        className="pointer-events-none absolute bottom-0 right-1/4 w-125 h-100 rounded-full blur-3xl opacity-15"
        style={{
      ...
      <div className="w-full max-w-7xl mx-auto relative">
        ...
        <motion.div
          ...
          initial="hidden"
          animate="show"
          className="text-center pt-6 sm:pt-8 md:pt-10 mb-10 sm:mb-12 md:mb-14 lg:mb-16"
        >
          <motion.div
            variants={fadeUp}
          ...
          <motion.p
            variants={fadeUp}
            className="text-xs sm:text-sm md:text-base lg:text-lg text-slate-400 max-w-2xl mx-auto mb-6 sm:mb-8 md:mb-10"
          >
            Structured learning paths from absolute beginner to advanced trader
            — built for the Indian market, explained in plain English.
          </motion.p>
          {/* Stats strip */}
          <motion.div
            variants={fadeUp}
            className="grid grid-cols-2 min-[420px]:grid-cols-4 justify-items-center gap-3 sm:gap-6"
          >
            {[
        ...
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 mb-8 sm:mb-10">
          {TRACKS.map((t) => {
            ...
            return (
              ...
              >
                <div
                  className={`p-2 sm:p-2.5 rounded-xl border shrink-0 ${active ? t.bg : "bg-slate-800/40 border-slate-700/30"}`}
                >
                  <TIcon
                    size={14}
                    className={active ? t.color : "text-slate-500"}
                  />
                </div>
                <div className="min-w-0 flex-1">
                  <div
                    className={`text-[11px] sm:text-[12px] font-black uppercase tracking-wider truncate ${active ? t.color : "text-slate-400"}`}
                  >
                    {t.label}
                  </div>
                  <div className="text-[9px] sm:text-[10px] text-slate-600 mt-0.5">
                    {t.modules} modules · {t.hours}
                  </div>
                </div>
                {active && (
                  <ChevronRight
                    size={12}
                    className={`ml-auto shrink-0 ${t.color}`}
                  />
                )}
              </button>
            );
        ...
        >
          ...
          >
            <div>
              ...
              <p className="text-slate-500 text-xs sm:text-sm">
                Quick-reference definitions for every term you&apos;ll encounter
              </p>
            </div>
            <input
          ...
          <motion.div
            variants={fadeUp}
            className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 sm:gap-3 md:gap-4"
          >
            {filteredGlossary.map((g) => (
              <div
                key={g.term}
                className="p-4 rounded-2xl bg-[#0b1120] border border-slate-800/60 hover:border-slate-700 transition-all"
              >
        ...
        >
          <motion.div variants={fadeUp} className="mb-6">
            ...
            <p className="text-slate-500 text-sm">
              Common questions from new investors and traders
            </p>
          </motion.div>
          <motion.div variants={fadeUp} className="space-y-3">
            {FAQS.map((faq, i) => (
              ...
              >
                ...
                >
                  <span className="text-[13px] font-semibold text-slate-200 pr-4">
                    {faq.q}
                  </span>
                  <ChevronDown
                    size={15}
                    className={`shrink-0 text-slate-500 transition-transform duration-200 ${openFaq === i ? "rotate-180" : ""}`}
                  />
                </button>
                <AnimatePresence initial={false}>
                  {openFaq === i && (
                    ...
                    >
                      <p className="px-4 pb-4 text-[12px] text-slate-500 leading-relaxed border-t border-slate-800/60 pt-3">
                        {faq.a}
                      </p>
                    </motion.div>
                  )}
        ...
        <motion.div
          ...
          style={{
            background:
              "linear-gradient(135deg, #0d1a2e 0%, #0f1e35 50%, #101628 100%)",
          }}
        >
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse at center, #f59e0b 0%, transparent 65%)",
            }}
          />
          <GraduationCap
            size={32}
            className="text-amber-400 mx-auto mb-3 sm:mb-4 relative"
          />
          <h3 className="text-xl sm:text-3xl font-black text-white mb-2 sm:mb-3 relative">
            Ready to start learning?
          </h3>
          <p className="text-slate-400 mb-5 sm:mb-6 max-w-md mx-auto text-xs sm:text-sm relative">
            Begin with the Beginner track — completely free. Unlock Intermediate
            and Advanced as you progress.
          </p>
          <button
