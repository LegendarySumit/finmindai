...
const initialNews: NewsItem[] = [
  {
    ...
    summary:
      "Fed Chairman hints at potential rate reductions in Q2, boosting market sentiment across sectors. Bond yields drop sharply as traders price in two cuts for the second half of the year.",
    sentiment: "positive",
    time: "5 mins ago",
  ...
  },
  {
    ...
    summary:
      "Major tech companies exceed expectations, driven by AI infrastructure investments. Cloud revenues surge 40% YoY as enterprise adoption accelerates.",
    sentiment: "positive",
    time: "12 mins ago",
  ...
  },
  {
    ...
    summary:
      "Crude oil jumps 5% as OPEC announces production cuts, raising inflation fears. Energy sector outperforms as WTI crosses $90 threshold.",
    sentiment: "negative",
    time: "25 mins ago",
  ...
  },
  {
    ...
    summary:
      "Bitcoin and Ethereum surge as sources suggest imminent SEC approval for spot ETFs. Altcoins follow with broad market rally.",
    sentiment: "positive",
    time: "32 mins ago",
  ...
  },
  {
    ...
    summary:
      "Consumer spending data shows unexpected decline, weighing on retail stocks. Holiday season forecasts revised downward.",
    sentiment: "negative",
    time: "45 mins ago",
  ...
  },
  {
    ...
    summary:
      "Key chip manufacturers warn of extended lead times as AI demand skyrockets globally. TSMC capacity utilization hits 98%.",
    sentiment: "neutral",
    time: "52 mins ago",
...
const ImpactBar = ({
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[9px] text-slate-600 uppercase tracking-widest w-16">
        Impact
      </span>
...
const CompactCard = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute top-0 left-0 bottom-0 w-0.5 bg-linear-to-b ${cfg.stripe} opacity-60 group-hover:opacity-100 transition-opacity`}
      />
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between mb-3 pl-2">
          ...
          >
            <span
              className={`w-1 h-1 rounded-full ${cfg.dot} ${news.isNew ? "animate-ping" : ""}`}
            />
            {cfg.label}
          </span>
        ...
        <div className="flex gap-1 pl-2 mb-4 flex-wrap">
          {news.impactedStocks.slice(0, 3).map((s) => (
            <span
              key={s}
              className="px-2 py-0.5 bg-finance-darker border border-finance-border/40 rounded text-[9px] font-mono font-bold text-slate-400"
            >
              ${s}
            </span>
          ))}
...
const MarketIntelligenceReal = () => {
  ...
  useEffect(() => {
    if (lastMessage && lastMessage.type === "news_update") {
      ...
      const title =
        typeof payload.title === "string" ? payload.title : "Market update";
  ...
  const filtered =
    sentimentFilter === "all"
      ? newsFeed
      : newsFeed.filter((n) => n.sentiment === sentimentFilter);
  ...
  return (
    <section
      id="news"
      className="py-10 sm:py-14 md:py-20 lg:py-24 bg-finance-dark relative overflow-hidden px-2 sm:px-3 md:px-4 lg:px-6 xl:px-8"
    >
      {/* Background orbs */}
      <motion.div
        ...
        <div className="text-center mb-10 sm:mb-12 md:mb-14 lg:mb-16">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-finance-gold/10 border border-finance-gold/20 rounded-full">
              ...
              <span className="text-xs text-finance-gold font-black uppercase tracking-[0.2em]">
                Market Intel
              </span>
            </div>
            ...
            {/* Live/offline pill */}
            <div
              className={`flex items-center gap-2 px-3 py-2 rounded-full text-xs font-bold border ${
                isConnected
          ...
          <div className="overflow-hidden flex-1">
            ...
            >
              {[...newsFeed, ...newsFeed].map((n, i) => (
                ...
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${SENTIMENT_CONFIG[n.sentiment].dot}`}
                  />
                ...
                <span
                  className={`ml-0.5 text-[9px] px-1.5 py-0.5 rounded-full font-black ${
                    isActive
                      ...
                      : "bg-finance-darker text-slate-500"
                  }`}
                >
                  {counts[f]}
                </span>
          ...
          <div className="mt-8 text-center">
            ...
            {hasMoreNews && (
              <p className="text-xs text-slate-500 mt-3">
                Showing top {MAX_HOME_NEWS} signals here. Open News Intel for
                the full feed.
              </p>
            )}
          </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  {[
                    ...
                    {
                      label: "Impact Score",
                      value: `${selectedNews.impactScore}/10`,
                    },
                    {
                      label: "Market Reaction",
                      value:
                        selectedNews.impactScore >= 8
                          ? "HIGH VOLATILITY"
                          : selectedNews.impactScore >= 6
                            ? "MODERATE VOLATILITY"
                            : "LOW VOLATILITY",
                    },
                    {
                    ...
                    <div
                      key={idx}
                      className="flex items-center justify-between p-3 rounded-lg bg-finance-card/50 border border-finance-border/30"
                    >
                      <span className="text-xs font-bold text-slate-400 uppercase">
                        {item.label}
                      </span>
                      <span
                        className={`text-sm font-black ${selectedNews.sentiment === "positive" ? "text-emerald-400" : selectedNews.sentiment === "negative" ? "text-rose-400" : "text-blue-400"}`}
                      >
                        {item.value}
                      </span>
                    </div>
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-2">
                    Risk indicator based on impact score and volatility
                  </p>
                </div>
          ...
          >
            ...
            <div className="p-6 pb-4 border-b border-finance-border/30">
              <div className="flex items-center gap-2 mb-3">
                <span
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border ${SENTIMENT_CONFIG[selectedNews.sentiment].badge}`}
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${SENTIMENT_CONFIG[selectedNews.sentiment].dot}`}
                  />
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-lg font-black text-finance-gold">
                    {selectedNews.impactScore}/10
                  </span>
                </div>
                <div className="h-2 bg-finance-card rounded-full overflow-hidden">
                  <motion.div
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedNews.impactedStocks.map((stock) => (
                    <span
                      key={stock}
                      className="px-2.5 py-1 bg-finance-card border border-finance-border rounded-lg text-[10px] font-bold text-slate-300"
                    >
                      ${stock}
                    </span>
                  ))}
