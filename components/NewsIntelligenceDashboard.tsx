...
import {
    Activity, BarChart3, Filter, Search, Pause, Play, RotateCcw,
    Share2, Bookmark, ArrowRight, Zap, Clock, TrendingUp, TrendingDown,
    Radio, RefreshCw, ExternalLink, ChevronRight, CheckCircle2,
    Newspaper,
} from 'lucide-react';
...
const NewsIntelligenceDashboard = () => {
    ...
    const [analysisByNewsId, setAnalysisByNewsId] = useState<Record<number, DeepAnalysisResult>>({});
    ...
    const generateDeepAnalysis = useCallback(async () => {
        ...
        try {
            ...
            const message = err instanceof Error ? err.message : 'Failed to generate deep analysis';
    ...
    const sentimentStats = useMemo(() => {
        ...
        newsFeed.forEach(n => { s[n.sentiment]++; });
        return s;
    }, [newsFeed]);
    ...
    const filtered = useMemo(() => {
        ...
        return newsFeed.filter(n =>
            (!q || n.title.toLowerCase().includes(q) || n.aiInsight.toLowerCase().includes(q) || n.source.toLowerCase().includes(q))
            && (selectedCategory === 'All' || n.category === selectedCategory)
            && (sentimentFilter === 'all' || n.sentiment === sentimentFilter)
        );
    }, [newsFeed, searchQuery, selectedCategory, sentimentFilter]);

    const activeAnalysis = selectedNews ? analysisByNewsId[selectedNews.id] : null;
    const isGeneratingAnalysis = !!selectedNews && analysisLoadingId === selectedNews.id;
    ...
    return (
        <div className="flex flex-col min-h-full lg:h-full bg-[#060b14] text-slate-200 overflow-visible lg:overflow-hidden font-sans">
            ...
            <div className="bg-[#0a0f1c] border-b border-slate-800/60 h-9 flex items-center overflow-hidden whitespace-nowrap relative z-30 shrink-0">
                ...
                <div className="overflow-hidden flex-1 relative">
                    {newsFeed.length > 0 ? (
                        ...
                        >
                            {[...newsFeed.slice(0, 10), ...newsFeed.slice(0, 10)].map((n, i) => (
                                <span key={i} className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-slate-500 shrink-0">
                                    <span className={`w-1.5 h-1.5 rounded-full ${SENTIMENT[n.sentiment].dot}`} />
                                    <span className="font-bold text-slate-600 font-mono text-[8px] sm:text-[9px] uppercase">[{n.source}]</span>
                                    <span className="text-slate-300 max-w-35 sm:max-w-none truncate">{n.title}</span>
                                    <span className="text-slate-700 mx-1">{"\u2022"}</span>
                                </span>
                            ))}
                        </motion.div>
                    ) : (
                        <span className="text-[11px] text-slate-600 pl-4 italic">Fetching live market intelligence...</span>
                    )}
                </div>
                {lastFetched && (
            ...
            <div className="flex flex-1 flex-col lg:flex-row overflow-visible lg:overflow-hidden relative">
                <div className="absolute top-0 right-0 w-96 h-96 rounded-full blur-3xl pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(251,191,36,0.03) 0%, transparent 70%)' }} />
                <div className="absolute bottom-0 left-0 w-72 h-72 rounded-full blur-3xl pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(59,130,246,0.04) 0%, transparent 70%)' }} />
                ...
                <aside className="w-full lg:w-72 lg:shrink-0 bg-[#0a0f1c]/90 border-b lg:border-b-0 lg:border-r border-slate-800/60 flex flex-col overflow-visible lg:overflow-y-auto custom-scrollbar z-20 max-h-none">
                    <div className="p-4 sm:p-5 border-b border-slate-800/50">
                        ...
                        <div className="flex items-center gap-2 text-xs">
                            <span className="relative flex h-2 w-2">
                                <span className={`animate-ping absolute h-full w-full rounded-full opacity-75 ${fetchError ? 'bg-rose-500' : 'bg-emerald-500'}`} />
                                <span className={`relative inline-flex rounded-full h-2 w-2 ${fetchError ? 'bg-rose-500' : 'bg-emerald-500'}`} />
                            </span>
                            <span className={fetchError ? 'text-rose-400' : isLoading ? 'text-amber-400' : 'text-emerald-400'}>
                                {fetchError ? 'Feed error — retrying...' : isLoading ? 'Fetching signals...' : `${newsFeed.length} signals loaded`}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="p-4 sm:p-5 border-b border-slate-800/50">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Market Sentiment</span>
                        </div>
                        <div className="bg-[#111827] rounded-2xl p-4 border border-slate-800">
                            <div className="h-32 sm:h-36 relative flex items-center justify-center">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                    <span className="text-2xl font-black text-white">{newsFeed.length}</span>
                                    <span className="text-[9px] text-slate-600 uppercase tracking-widest font-bold">signals</span>
                                </div>
                            </div>
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`text-center p-2 rounded-xl border transition-all ${
                                                isActive
                                                    ? s === 'positive' ? 'bg-emerald-950/40 border-emerald-500/40'
                                                        : s === 'negative' ? 'bg-rose-950/40 border-rose-500/40'
                                                        : 'bg-blue-950/40 border-blue-500/40'
                                                    : 'bg-slate-900/50 border-slate-800/50 hover:border-slate-700'
                                            }`}
                                        >
                                            <div className={`font-black text-base leading-none mb-1 ${cfg.text}`}>{sentimentStats[s]}</div>
                                            <div className={`text-[9px] font-black uppercase ${cfg.text} opacity-60`}>{cfg.label.slice(0, 4)}</div>
                                        </button>
                                    );
                                })}
                            </div>
                            ...
                            {CATEGORIES.map(cat => (
                                ...
                                >
                                    ...
                                    <span className={`text-[9px] px-2 py-0.5 rounded-full font-black ${
                                        selectedCategory === cat ? 'bg-amber-500/20 text-amber-400' : 'bg-slate-800 text-slate-600'
                    ...
                    <div className="p-4 border-t border-slate-800/50">
                        <div className="text-[9px] text-slate-700 font-mono text-center space-y-1">
                            ...
                            <div>Auto-refresh: <span className="text-amber-700">every 90s</span></div>
                            <div>Status: <span className={fetchError ? 'text-rose-600' : 'text-emerald-700'}>{fetchError ? 'DEGRADED' : 'OPERATIONAL'}</span></div>
                        </div>
                    </div>
                </aside>
                ...
                <main className="flex-1 flex flex-col min-w-0 overflow-visible lg:overflow-hidden">
                    <header className="px-3 sm:px-5 py-3 border-b border-slate-800/60 flex flex-wrap items-center gap-2 sm:gap-3 bg-[#060b14]/80 backdrop-blur shrink-0 z-10">
                        <div className="relative flex-1 min-w-0 max-w-full lg:max-w-lg">
                            ...
                            />
                            {searchQuery && (
                                <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 hover:text-white text-sm">{"\u2715"}</button>
                            )}
                        </div>
                        <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto justify-end">
                            {filtered.length > 0 && (
                                <span className="text-[10px] text-slate-600 font-mono hidden xl:block">{filtered.length} signals</span>
                            )}
                            <button
                            ...
                            <button
                                onClick={() => { setNewsFeed([]); setSelectedNews(null); fetchNews(true); }}
                        ...
                        <AnimatePresence initial={false} mode="popLayout">
                            {isLoading && newsFeed.length === 0 ? (
                                <div key="skeletons" className="space-y-2.5">
                                    {Array.from({ length: 7 }).map((_, i) => <SkeletonCard key={i} i={i} />)}
                                </div>
                            ) : filtered.length === 0 ? (
                                <motion.div
                                    key="empty"
                                ...
                                >
                                    ...
                                    <p className="text-base font-bold text-slate-500 mb-1">
                                        {newsFeed.length === 0 ? 'Awaiting Intelligence Feed' : 'No matching signals'}
                                    </p>
                                    <p className="text-xs text-slate-700 mb-6">
                                        {newsFeed.length === 0 ? 'Connecting to live market data...' : 'Try adjusting your filters'}
                                    </p>
                                    {newsFeed.length === 0 && (
                                        <button
                                            onClick={() => fetchNews(true)}
                                ...
                                filtered.map(news => {
                                    ...
                                    return (
                                        <motion.div
                                            ...
                                            onClick={() => setSelectedNews(isSelected ? null : news)}
                                        ...
                                        >
                                            ...
                                            {isNew && (
                                                <div className="absolute top-3 right-3 flex items-center gap-1 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full">
                                                    ...
                                                    <span className="text-[9px] font-black text-emerald-400 uppercase tracking-wider">New</span>
                                                </div>
                                            )}
                                            <div className="flex items-center gap-2.5 mb-2.5">
                                                ...
                                                <span className="text-[10px] text-slate-600 font-medium">{news.source}</span>
                                            ...
                                            <div className="flex items-center justify-between gap-3">
                                                <div className="flex items-center gap-2 flex-1 min-w-0">
                                                    <span className="text-[9px] text-slate-700 uppercase shrink-0">Impact</span>
                                                    ...
                                                    <span className={`text-[10px] font-black shrink-0 ${cfg.text}`}>{news.impactScore}</span>
                                                </div>
                                                <div className="flex items-center gap-2 shrink-0">
                                                    <span className="text-[9px] px-2 py-0.5 rounded-md bg-slate-800/80 text-slate-500 font-bold">
                                                        {news.category}
                                                    </span>
                                                    <SentIcon size={11} className={`opacity-0 group-hover:opacity-100 transition-opacity ${cfg.text}`} />
                                                </div>
                                            </div>
                                        </motion.div>
                                    );
                ...
                {/* RIGHT PANEL */}
                <aside ref={analysisPanelRef} className="w-full xl:w-96 xl:shrink-0 bg-[#0a0f1c]/90 border-t xl:border-t-0 xl:border-l border-slate-800/60 flex flex-col overflow-visible xl:overflow-hidden z-20 max-h-none">
                        ...
                        {selectedNews ? (
                            ...
                            >
                                ...
                                <div className="flex-1 overflow-visible xl:overflow-y-auto p-3 sm:p-5 custom-scrollbar space-y-4 sm:space-y-5">
                                    <div className="flex items-center gap-2 flex-wrap">
                                        ...
                                        <span className="text-[11px] sm:text-xs text-slate-500 font-medium">{selectedNews.source}</span>
                                        <span className="w-full min-[420px]:w-auto min-[420px]:ml-auto text-[11px] sm:text-xs text-slate-600">{selectedNews.timestamp}</span>
                                    </div>
                                    <h2 className="text-base sm:text-lg font-black text-white leading-tight">{selectedNews.title}</h2>
                                    <div className="bg-[#111827] border border-slate-800 rounded-xl sm:rounded-2xl p-4 sm:p-5 relative overflow-hidden">
                                        <div className="absolute top-0 right-0 opacity-[0.03] pointer-events-none"><Zap size={110} /></div>
                                        <div className="flex items-center gap-2 mb-3">
                                            <Zap size={11} className="text-amber-500" />
                                            <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">AI Market Context</span>
                                        </div>
                                        <p className="text-sm text-slate-300 leading-relaxed">{selectedNews.aiInsight}</p>
                                        <div className="mt-4 sm:mt-5 pt-4 sm:pt-5 border-t border-slate-800 grid grid-cols-1 min-[420px]:grid-cols-2 gap-3 sm:gap-5">
                                            <div className="min-w-0">
                                                <div className="text-[9px] text-slate-600 font-bold uppercase tracking-wider mb-2">Impact Score</div>
                                                <div className="text-xl sm:text-2xl font-black text-white">{selectedNews.impactScore}<span className="text-sm text-slate-500 font-medium">/10</span></div>
                                            ...
                                            <div className="min-w-0">
                                                <div className="text-[9px] text-slate-600 font-bold uppercase tracking-wider mb-2">Category</div>
                                                <div className="text-base sm:text-xl font-black text-white wrap-break-word leading-tight">{selectedNews.category}</div>
                                                <div className="text-[10px] text-slate-600 mt-2 flex items-center gap-1">
                                                    <CheckCircle2 size={10} className="text-emerald-600" /> Verified source
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="bg-[#0d1320] border border-slate-800/50 rounded-xl p-3 sm:p-4">
                                        <div className="text-[10px] font-black text-slate-600 uppercase tracking-wider mb-3">Potentially Affected</div>
                                        <div className="space-y-1.5">
                                            {(['AAPL', 'NVDA', 'MSFT'] as const).map((ticker, i) => (
                                                <div key={ticker} className="flex items-center justify-between p-2 sm:p-2.5 hover:bg-slate-800/30 rounded-lg transition-all cursor-pointer group">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <div className="text-xs font-bold text-slate-300 group-hover:text-white transition-colors">{ticker}</div>
                                                            <div className="text-[9px] text-slate-600">Tech Sector</div>
                                                        </div>
                                                    </div>
                                                    <span className={`text-xs font-black font-mono ${i === 1 ? 'text-rose-400' : 'text-emerald-400'}`}>
                                                        {i === 1 ? '-0.8%' : '+1.2%'}
                                                    </span>
                                                </div>
                                            ))}
                                    ...
                                    <div className="bg-[#0d1320] border border-slate-800/50 rounded-xl p-3 sm:p-4 space-y-3">
                                        <div className="flex items-center justify-between gap-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-[10px] font-black text-amber-500 uppercase tracking-wider">Deep Analysis Guide</span>
                                            </div>
                                            {activeAnalysis && (
                                                <span className="text-[10px] text-slate-500 font-mono">{activeAnalysis.confidence}% conf</span>
                                            )}
                                        </div>
                                        ...
                                        {!isGeneratingAnalysis && activeAnalysis && (
                                            <div className="space-y-3">
                                                <p className="text-xs text-slate-300 leading-relaxed">{activeAnalysis.thesis}</p>

                                                <div>
                                                    <div className="text-[9px] font-black text-slate-600 uppercase tracking-wider mb-2">Immediate Actions</div>
                                                    <div className="space-y-1.5">
                                                        {activeAnalysis.immediateActions.slice(0, 3).map((step, idx) => (
                                                            <div key={`${idx}-${step.slice(0, 16)}`} className="text-[11px] text-slate-400 bg-slate-900/40 border border-slate-800/60 rounded-lg p-2.5">
                                                                {idx + 1}. {step}
                                                            </div>
                                                        ))}
                                                    </div>
                                                ...
                                                <div>
                                                    <div className="text-[9px] font-black text-slate-600 uppercase tracking-wider mb-2">Stock Plan</div>
                                                    <div className="space-y-2">
                                                        {activeAnalysis.stockIdeas.slice(0, 3).map((idea) => (
                                                            <div key={idea.symbol} className="p-2.5 rounded-lg border border-slate-800/60 bg-slate-900/30">
                                                                <div className="flex items-center justify-between gap-2 mb-1.5">
                                                                    <span className="text-xs font-black text-white">{idea.symbol}</span>
                                                                    <span className={`text-[10px] font-black uppercase ${idea.bias === 'up' ? 'text-emerald-400' : idea.bias === 'down' ? 'text-rose-400' : 'text-blue-400'}`}>
                                                                        {idea.bias}
                                                                    </span>
                                                                </div>
                                                                <div className="text-[11px] text-slate-400">{idea.rationale}</div>
                                                                {(idea.price || idea.changePercent !== null) && (
                                                                    <div className="mt-1.5 text-[10px] text-slate-500 font-mono">
                                                                        {idea.price ? `$${idea.price.toFixed(2)}` : 'N/A'} {typeof idea.changePercent === 'number' ? `• ${idea.changePercent >= 0 ? '+' : ''}${idea.changePercent.toFixed(2)}%` : ''}
                                                                    </div>
                                                                )}
                                                            </div>
                                                        ))}
                                                ...
                                                <div>
                                                    <div className="text-[9px] font-black text-slate-600 uppercase tracking-wider mb-2">Execution Horizon</div>
                                                    <div className="grid grid-cols-1 gap-1.5">
                                                        <div className="text-[11px] text-slate-400 bg-slate-900/40 border border-slate-800/60 rounded-lg p-2.5">
                                                            <span className="text-slate-300 font-bold">Intraday:</span> {activeAnalysis.planByHorizon.intraday[0] || 'Wait for high-quality setups only.'}
                                                        </div>
                                                        <div className="text-[11px] text-slate-400 bg-slate-900/40 border border-slate-800/60 rounded-lg p-2.5">
                                                            <span className="text-slate-300 font-bold">Swing:</span> {activeAnalysis.planByHorizon.swing[0] || 'Scale in gradually with confirmation.'}
                                                        </div>
                                                        <div className="text-[11px] text-slate-400 bg-slate-900/40 border border-slate-800/60 rounded-lg p-2.5">
                                                            <span className="text-slate-300 font-bold">Position:</span> {activeAnalysis.planByHorizon.position[0] || 'Reassess thesis weekly against macro changes.'}
                                                        </div>
                                                    </div>
                                                </div>

                                                <div>
                                                    <div className="text-[9px] font-black text-slate-600 uppercase tracking-wider mb-2">Scenario Outlook</div>
                                                    <div className="space-y-1.5">
                                                        {activeAnalysis.scenarioMap.slice(0, 2).map((scenario) => (
                                                            <div key={scenario.scenario} className="text-[11px] text-slate-400 bg-slate-900/40 border border-slate-800/60 rounded-lg p-2.5">
                                                                <span className="text-slate-300 font-bold">{scenario.scenario}</span> ({scenario.probability}%): {scenario.expectedMove}
                                                            </div>
                                                        ))}
                                                    </div>
                                                </div>
                                                ...
                                                <div>
                                                    <div className="text-[9px] font-black text-slate-600 uppercase tracking-wider mb-2">Risk Rules</div>
                                                    <div className="space-y-1.5">
                                                        {activeAnalysis.riskRules.slice(0, 3).map((rule, idx) => (
                                                            <div key={`${idx}-${rule.slice(0, 14)}`} className="text-[11px] text-slate-400 bg-slate-900/40 border border-slate-800/60 rounded-lg p-2.5">
                                                                {idx + 1}. {rule}
                                                            </div>
                                                        ))}
                                                    </div>
                                                ...
                                                <div className="text-[10px] text-slate-500 border-t border-slate-800/60 pt-2">{activeAnalysis.disclaimer}</div>
                                            </div>
                                        )}

                                        {!isGeneratingAnalysis && !activeAnalysis && !analysisError && (
                                            <p className="text-[11px] text-slate-500 leading-relaxed">
                                                Generate a structured plan for this specific headline: market bias, stock selection, scenarios, and risk controls.
                                            </p>
                                        )}
                                    </div>
                                </div>
                            ...
                            >
                                <div className="px-5 py-4 border-b border-slate-800/60">
                                    <div className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Intelligence Panel</div>
                                </div>
                                <div className="p-4 grid grid-cols-3 gap-3 border-b border-slate-800/50">
                                    {[
                                        { label: 'Bullish', count: sentimentStats.positive, color: 'text-emerald-400', bg: 'bg-emerald-950/30 border-emerald-500/10' },
                                        { label: 'Bearish', count: sentimentStats.negative, color: 'text-rose-400', bg: 'bg-rose-950/30 border-rose-500/10' },
                                        { label: 'Neutral', count: sentimentStats.neutral, color: 'text-blue-400', bg: 'bg-blue-950/30 border-blue-500/10' },
                                    ].map(s => (
                                        <div key={s.label} className={`rounded-xl border p-3 text-center ${s.bg}`}>
                                            <div className={`text-xl font-black ${s.color}`}>{s.count}</div>
                                            <div className={`text-[9px] font-black uppercase ${s.color} opacity-60`}>{s.label}</div>
                                        </div>
                                    ))}
                                </div>
                                {newsFeed.length > 0 ? (
                                    <div className="p-4 flex-1 overflow-visible xl:overflow-y-auto custom-scrollbar">
                                        <div className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-3">Top Signals</div>
                                        <div className="space-y-1">
                                            {[...newsFeed].sort((a, b) => b.impactScore - a.impactScore).slice(0, 8).map(news => {
                                                const cfg = SENTIMENT[news.sentiment];
                                                return (
                                                    <button
                                                        key={news.id}
                                                    ...
                                                    >
                                                        <span className={`mt-1 w-2 h-2 rounded-full ${cfg.dot} shrink-0`} />
                                                        <div className="min-w-0 flex-1">
                                                            <p className="text-xs font-semibold text-slate-400 line-clamp-2 leading-snug group-hover:text-white transition-colors">{news.title}</p>
                                                            <span className={`text-[9px] font-bold mt-0.5 block ${cfg.text}`}>
                                                                {news.impactScore}/10 {"\u2022"} {news.source}
                                                            </span>
                                                        </div>
                                                        <ChevronRight size={12} className="text-slate-700 group-hover:text-amber-500 shrink-0 mt-0.5 transition-colors" />
                                                    </button>
                                                );
                                            })}
                                        </div>
                                    ...
                                    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                                        ...
                                        <h3 className="text-sm font-black text-slate-500 mb-2">Select a Signal</h3>
                                        <p className="text-xs text-slate-700 leading-relaxed">Click a news card to view AI analysis, market impact, and related assets.</p>
                                    </div>
                                )}
                            </motion.div>
                        )}
