...
const sanitizeActivityValue = (value: unknown): ActivityMetadataValue | undefined => {
  ...
  if (typeof value === 'object') {
    ...
    return Object.fromEntries(sanitizedEntries) as { [key: string]: ActivityMetadataValue };
  }
...
const getActivitySessionId = () => {
  ...
  try {
    const existingSessionId = window.sessionStorage.getItem(ACTIVITY_SESSION_STORAGE_KEY);
...
const deriveAuthType = (firebaseUser: FirebaseUser, profileAuthType?: string): UserAuthType => {
  if (profileAuthType === 'wallet' || profileAuthType === 'google' || profileAuthType === 'email') {
...
const isPermissionDeniedError = (err: unknown) => {
  ...
  if (err instanceof Error) {
    return err.message.toLowerCase().includes('missing or insufficient permissions');
  }
...
const buildUserFromFirebase = async (firebaseUser: FirebaseUser): Promise<User> => {
  ...
  return {
    id: firebaseUser.uid,
    address: firebaseUser.uid.startsWith('wallet_') ? `0x${firebaseUser.uid.slice(7)}` : undefined,
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Firebase is not configured.';
  ...
  const loginWithWallet = useCallback(async () => {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Wallet login failed';
      setError(errorMsg);
      throw err;
    } finally {
      setIsLoading(false);
  ...
  const trackActivity = useCallback(async (type: string, metadata: Record<string, unknown> = {}) => {
    if (!user) return;
